...
| 파일 | 역할 |
| --- | --- |
| `notebooks/04_regression_modeling.ipynb` | 회귀 모델링 실험과 결과 확인 |
| `src/regression.py` | Open Source SW 요구사항을 위한 top-level function 구현 |
| `reports/regression_modeling_user_manual.md` | 함수 사용법, 파라미터, 출력 설명 |
...
## 10. Open Source SW 함수화

과제 요구사항의 Open Source SW 항목에서는 반복 코드를 하나의 top-level function으로 정리하고, function manual/specification을 제공할 것을 요구한다. 이를 충족하기 위해 `src/regression.py`에 `run_regression_modeling()` 함수를 추가했다.

이 함수는 다음 작업을 한 번에 수행한다.

- `data/processed/hotel_bookings_reg.csv` 로드
- `adr`, `is_canceled`, `assigned_room_type_*` 제외
- train/test split
- 5개 모델/파라미터 조합의 5-fold cross validation
- CV RMSE 기준 최종 모델 선택
- test set 평가
- 발표용 figure 저장

함수 사용법과 파라미터 설명은 `reports/regression_modeling_user_manual.md`에 정리했다. 따라서 최종 제출 시 노트북 결과뿐 아니라 재사용 가능한 source code와 user manual도 함께 제시할 수 있다.
