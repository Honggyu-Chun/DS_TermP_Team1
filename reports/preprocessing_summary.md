...
* **Removal of "Ghost Bookings"**: Purged **180 invalid entries** where the sum of adults, children, and babies was zero, as these represent non-existent guest profiles.
...
* **Target-Specific Filtering**:
    * **Regression Focus**: A strict range of $0 < ADR < 500$ was applied to the **Regression dataset** to ensure target stability and remove extreme outliers.
...
* **Numeric Verification**: The final dataset was audited to ensure exactly **zero `object` or `string` columns** remain, resulting in a finalized count of **83 numeric features**.
...
| :--- | :--- | :--- | :--- |
| **Total Rows** | 119,390 | **119,210** | **~74,000*** |
| **Total Columns** | 32 | **83** | **83** |
| **Missing Values** | 129,431+ | 0 | 0 |
| **Object Columns** | 12 | 0 | 0 |
*\*The REG dataset consists exclusively of non-canceled bookings with strict ADR filtering ($0 < ADR < 500$).*

---

## 7. Conclusion
Through this preprocessing pipeline, we successfully cleaned the data noise and expanded the feature set from 32 to **83 fully digitized columns**. By synchronizing the logic between the Python script and the Jupyter notebook, we have established a reproducible and academically rigorous foundation for the subsequent modeling phase.