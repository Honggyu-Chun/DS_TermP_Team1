...
Decision Tree는 F1-score가 0.7737로 가장 높고 recall도 0.7371로 가장 좋았다. 프로젝트 목적이 취소 위험 예약을 사전에 식별하는 것이므로, precision만 높은 모델보다 precision과 recall의 균형이 좋은 Decision Tree를 최종 모델로 선택했다.
...
8. Decision Tree가 CV F1 0.7737로 가장 높아 최종 모델이 되었다.
...
따라서 현재 결과는 **분류 모델링 파트의 1차 완성본으로 사용 가능**하다. 발표에서는 “Decision Tree가 Random Forest보다 accuracy는 약간 낮지만, 취소 예약을 더 잘 찾아내는 recall과 F1-score가 높아 최종 선택했다”는 논리로 설명하면 된다.
